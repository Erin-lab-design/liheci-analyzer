...
- **Stage 3**: 使用HFST替换规则标注插入成分，Python提取并分类插入语类型，基于覆盖率计算置信度，输出详细插入成分类型、错误类型、置信度分数（206条）
- **Stage 4**: HanLP分词+POS验证，自动过滤POS不符、PP/DE错误、低置信度候选，输出高精度最终结果（160条，Precision 92.36%）
...
- ✅ **Stage 3**: HFST字符级标注+Python插入成分类型分类，输出详细插入类型、错误类型、置信度分数（206条）
- ✅ **Stage 4**: HanLP分词+POS验证，自动过滤POS不符、PP/DE错误、低置信度候选，输出高精度最终结果（160条，Precision 92.36%）
...
# 输出: outputs/liheci_insertion_analysis.tsv (206 rows, 含插入类型/错误/置信度，自动过滤低置信度，默认阈值0.3)

# Stage 4: POS验证与高精度过滤
python scripts/07.stage4_pos_validation.py
# 输出: outputs/liheci_pos_validated.tsv (160 rows, 高精度)
```
...
206 rows（已自动过滤低置信度，默认阈值0.3），包含以下附加字段：
...
- HanLP分词+POS验证，自动过滤head/tail POS不符、PP/DE错误、低置信度候选
- 主要字段：所有Stage 3字段 + head_pos, tail_pos, pos_validation_status
- 结果：Precision 92.36%，Recall 97.32%，F1 94.77%
