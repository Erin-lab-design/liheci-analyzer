...
def run_hfst_redup_on_sentence(sentence, hfst_path):
    ...
    try:
        result = subprocess.run(
            [str(hfst_path), str(Path(__file__).parent / "hfst_files" / "liheci_redup.analyser.hfst")],
            input=sentence,
            capture_output=True,
            text=True,
            encoding='utf-8',
...
def validate_reduplication(stage1_output, stage2_output, hfst_path):
    ...
    valid_redup = {}  # (sent_id, lemma) -> HFST analysis string
        ...
        if analyses:
            ...
            for analysis in analyses:
                ...
                if '+REDUP' in analysis:
                    ...
                    valid_redup[(sent_id, lemma)] = analysis
        ...
        if is_potential_redup:
            if (sent_id, lemma) in valid_redup:
                ...
                for row in group_rows:
                    if row['shape'] == 'SPLIT':
                        row['shape'] = 'REDUP'
                        # Use HFST REDUP recognizer's actual output
                        row['hfst_analysis'] = valid_redup[(sent_id, lemma)]
...
def main():
    ...
    stage2_output = base_dir / "outputs" / "liheci_hfst_outputs.tsv"
