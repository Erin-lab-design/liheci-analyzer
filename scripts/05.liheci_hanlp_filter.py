...
def main():
    ...
    with OUT_TSV.open("w", encoding="utf-8", newline="") as fout:
        ...
        for row in hfst_rows:
            ...
            if is_redup:
                # REDUP: we trust HFST, but still want head / tail indices from HanLP
                category = "REDUP"
                redup_count += 1

                # Reuse the SPLIT logic to locate head / tail tokens
                ok_split, reason_split, pattern_split, head_idx_list, tail_idx_list = check_split(
                    lemma=lemma,
                    type_tag=type_tag,
                    head=head,
                    tail=tail,
                ...
                # For redup we treat it as valid whenever HFST found it
                is_valid = True
                pattern_type = pattern_split or "redup"
                if ok_split:
                    reason = "redup_from_hfst"
                else:
                    # we still keep it, but mark that POS pattern did not match our heuristic
                    reason = f"redup_from_hfst_but_{reason_split}"
