...
for idx, row in df.iterrows():
    ...
    if "Pseudo" in l_type or "Simplex" in l_type:
        # Type B: looser constraints (见/VA, 打/NN)
        head_tags = ['VV', 'VA', 'NN']
    elif "Modifier" in l_type:
        # Type C: permitted modifiers (帮/NN, 杀/NN)
        head_tags = ['NN', 'AD', 'JJ']
    else:
        # Type A: strict verb only
        head_tags = ['VV']
...
def analyze_sentence(text, target_lemma, file_handle):
    ...
    # --- PATH 1: WholeEntry to check fused tokens ---
    for word in words:
    ...
    # --- PATH 2: SplitEntry to check split tokens with possible insertions ---
    if not found:
        for rule in fst_split_rules:
            ...
            for i, (word, tag) in enumerate(zip(words, tags)):
                ...
                if is_head_match:
                    ...
                    # if "帮/NN"，而 allowed_tags 是 ['VV']，这里直接 continue
