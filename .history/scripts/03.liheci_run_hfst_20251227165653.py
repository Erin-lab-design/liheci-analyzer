...
- 对“句子”部分调用 liheci_recognizer.hfst 做句子级分析
- 解析 HFST 输出中的：
    - lemma
...
# 句子级分析器
HFST_FST_PATH = os.environ.get("LIHECI_SPLIT_FST", "liheci_recognizer.hfst")

# 只保留识别出离合词的句子的结构化输出
OUTPUT_TSV = "data/liheci_hfst_outputs.tsv"
...
def hfst_analyze_sentence(sentence: str, logger):
    """
    用 liheci_recognizer.hfst 对整句做 HFST 分析，
