...
def main():
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        # --- 基本 WHOLE / SPLIT 模式 ---
        lines.append(
            f"define {whole_pat_name} ?* {head_chars} {tail_chars} ?* ;"
        )
        ...
        if has_redup(notes):
            ...
            # 绑定 transducer
            lines.append(
                f'define {whole_tr_name} "{whole_upper}" : {whole_only_pat_name} ;'
            )
            lines.append(
                f'define {split_tr_name} "{split_upper}" : {split_only_pat_name} ;'
            )
            lines.append(
                f'define {redup_tr_name} "{redup_upper}" : {redup_pat_name} ;'
            )

            transducers_for_this.extend(
                [whole_tr_name, split_tr_name, redup_tr_name]
            )
        else:
            # 对没有重叠的 lemma，直接用 WholePat / SplitPat
            lines.append(
                f"define {whole_only_pat_name} {whole_pat_name} ;"
            )
            lines.append(
                f"define {split_only_pat_name} {split_pat_name} ;"
            )
            lines.append(
                f'define {whole_tr_name} "{whole_upper}" : {whole_only_pat_name} ;'
            )
