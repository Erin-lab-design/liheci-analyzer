...
def run_test_suite():
    ...
    with open(TEST_FILE, "r", encoding="utf-8") as f_in, \
        ...
        # 先写个头，然后立刻 flush 一下，保证 tail 能看到
        f_out.write("TEST REPORT | Liheci HFST+HanLP Analyzer (Blind Mode)\n")
        f_out.write("==================================================\n\n")
        f_out.flush()   # ⭐ 关键：立刻刷到磁盘
        ...
        for line in lines:
            ...
            f_out.write(
                f"Detected Analyses:    {detected_lemmas if detected_lemmas else 'None'}\n"
            )
            f_out.write(
                f"Expected:             {expect_true} | Actual Found: {actual_detected} | Status: {status}\n\n"
            )
            ...
            # 关键 2：每处理完一个用例，flush 一次，这样 tail -f 能实时看到
            f_out.flush()   # ⭐⭐

            # 进度条也顺便加上 flush
            print(f"\rProcessed {total} cases...", end="", flush=True)
