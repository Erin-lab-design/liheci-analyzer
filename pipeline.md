...
│     - Modifiers: 个, 好, 完, etc.                           │
│                                                              │
│ Output: outputs/liheci_hfst_outputs.tsv (325 rows)         │
...
│                                                              │
│ Results:                                                     │
│   ├─ Valid REDUP: 17 lemmas (34→17 rows deduplicated)      │
│   ├─ Invalid REDUP: 90 rows filtered                       │
│   └─ Non-REDUP: 189 rows preserved                         │
│                                                              │
│ Output: outputs/liheci_hfst_outputs_filtered.tsv (206 rows)│
│         outputs/liheci_redup_validation.log                 │
└─────────────────────────────────────────────────────────────┘
    ↓
Current Results: 206 valid liheci recognitions
    ├─ 189 non-reduplication cases (WHOLE or SPLIT)
    └─ 17 valid reduplication cases (REDUP)
    ↓
[Stage 3] HFST Insertion Type Recognition
...
Candidates with insertion type annotations
    ↓
[Stage 4] HFST Coarse-grained Filtering (Optional)
...
Initially filtered candidates
    ↓
[Stage 5] Python Fine-grained Validation
...
- `scripts/liheci_split.xfst` - Generated XFST file (404 lines, 663 states)
- `scripts/liheci_split.analyser.hfst` - Compiled HFST analyser (~7MB)
- `scripts/liheci_split.generator.hfst` - Compiled HFST generator (~7MB)
- `scripts/03.liheci_run_hfst.py` - Stage 1 runner script

### Stage 2: REDUP Validation
- `scripts/02.generate_liheci_redup_xfst.py` - Generates XFST rules for REDUP patterns
- `scripts/liheci_redup.xfst` - Generated XFST file (404 lines, 225 states)
- `scripts/liheci_redup.analyser.hfst` - Compiled HFST analyser (~7MB)
- `scripts/liheci_redup.generator.hfst` - Compiled HFST generator (~7MB)
- `scripts/04.liheci_validate_redup_hfst.py` - Stage 2 validator script

### Output Files
- `outputs/liheci_hfst_outputs.tsv` - Stage 1 output (325 rows)
- `outputs/liheci_hfst_outputs_filtered.tsv` - Stage 2 final output (206 rows)
- `outputs/liheci_hfst_run.log` - Stage 1 execution log
- `outputs/liheci_redup_validation.log` - Stage 2 validation log

## Valid REDUP Lemmas (17 total)
