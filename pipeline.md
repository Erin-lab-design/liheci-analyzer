...
This pipeline uses a four-stage HFST approach to recognize and validate Chinese separable verbs (离合词) in natural text.

**Test Data**: 284 test instances (149 True + 135 False cases with 7 error types)
...
│                                                              │
│ Process:                                                     │
│ 1. Read Stage 1 output (301 rows)                          │
│ 2. Deduplicate by (sent_id, lemma, shape)                  │
│ 3. Identify WHOLE+SPLIT cooccurrence → potential REDUP     │
...
│                                                              │
│ Results:                                                     │
│   ├─ Valid REDUP: 21 cases kept                            │
│   ├─ Invalid REDUP: 90 rows filtered (100% filtering rate) │
│   └─ Final output: 190 rows                                │
│                                                              │
│ Output: outputs/liheci_hfst_outputs.tsv (190 rows)         │
│         outputs/liheci_redup_validation.log                 │
└─────────────────────────────────────────────────────────────┘
    ↓
Stage 1+2 Results: 190 rows (TP=149, FP=41)
    ├─ Precision: 78.42%
    ├─ Recall: 100.00%
    └─ F1: 87.91%
    ↓
...
│        (after confidence threshold filtering, default ≥ 0.3)│
└─────────────────────────────────────────────────────────────┘
    ↓
Stage 3 Results: 167 rows (TP=146, FP=21, FN=3)
    ├─ Precision: 87.43%
    ├─ Recall: 97.99%
    ├─ F1: 92.41%
...
Stage 4: POS-Based Validation (HanLP)
    ...
    ├─ Input: outputs/liheci_insertion_analysis.tsv (167 rows)
    ├─ HanLP POS tagging for each sentence
    ├─ Validates head/tail POS by liheci type:
    ...
    ├─ Output: outputs/liheci_pos_validated.tsv (160 rows)
    └─ Rejected: outputs/liheci_pos_rejected.tsv (10 rows)
    ↓
Stage 4 Final Results: 157 unique predictions (TP=145, FP=12, FN=4)
    ├─ Precision: 92.36%
    ├─ Recall: 97.32%
    ├─ F1: 94.77%
    ├─ File output: 160 rows (some sentences have multiple matches)
    └─ Remaining FP: mostly COINCIDENCE type (12/12)
...
- `data/test_sentences.txt` - Test sentences (284 instances: 149 True + 135 False)
...
### Output Files
- `outputs/liheci_hfst_outputs.tsv` - Stage 1+2 output (190 rows)
- `outputs/liheci_insertion_analysis.tsv` - Stage 3 output (167 rows)
- `outputs/liheci_pos_validated.tsv` - Stage 4 final output (160 rows)
- `outputs/liheci_pos_rejected.tsv` - Stage 4 rejected (10 rows)
- `outputs/liheci_evaluation_report_by06.txt` - Stage 3 evaluation
- `outputs/liheci_evaluation_report_by07.txt` - Stage 4 evaluation
